...
% 68k training iterations: DQ (when we generated the samples)
...
\end{subfigure}
\caption[Comparison of Losses on UCF-101]{Comparison of validation results on MsPacman using similar network configurations with varying loss functions. All models have been trained given an input sequence of 10 patches of size $32 \times 32$ to predict the next 10 frames. The loss values shown represent the per-pixel error.} \label{fig:plot-ucf-loss}
\end{figure}
...
\begin{figure}[htpb]
	...
	\caption[TBD]{tbd.} \label{fig:ucf-comparison}
\end{figure}
...
\begin{figure}[htpb]
	...
	\caption[TBD]{tbd.} \label{fig:ucf-long1}
\end{figure}
