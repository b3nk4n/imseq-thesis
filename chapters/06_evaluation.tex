...
\begin{figure}[htb]
  ...
  \begin{tikzpicture}
    \begin{axis}[
      ...
      \addplot[draw=LightGoldenrod, thin] table[x=Step, y=Value]{\modelSS};\label{mSS}
    \end{axis}
    \begin{axis}[
    	axis y line*=left,
    	ymode=log,
...
\begin{figure}[htb]
  ...
  \begin{tikzpicture}
    ...
    \begin{axis}[
	  ...
	  \addlegendimage{/pgfplots/refstyle=mB}\addlegendentry{$\eta$-decay=0.90 (training)}
      \addlegendimage{/pgfplots/refstyle=mA}\addlegendentry{$\eta$-decay=0.95 (training)}
...
\begin{subfigure}{0.5\textwidth}
  ...
  \begin{tikzpicture}[scale=0.5]
    \begin{axis}[
      ...
      \addplot[draw=black!30!blue] table[x=Step, y=Value]{\modelB};
      \addlegendentry{32 ConvLSTM feature maps};
      %best
      \addplot[draw=black!30!green] table[x=Step, y=Value]{\modelA};
      \addlegendentry{64 ConvLSTM feature maps};
      
      \addplot[draw=black!30!red] table[x=Step, y=Value]{\modelC};
      \addlegendentry{128 ConvLSTM feature maps};
    \end{axis}
  \end{tikzpicture}
  }
  \caption{}
...
\begin{subfigure}{0.5\textwidth}
  ...
  \begin{tikzpicture}[scale=0.5]
    \begin{axis}[
      ...
      \addplot[draw=black!30!red] table[x=Step, y=Value]{\modelC};
      \addlegendentry{BCE+GDL+SSIM};
      \addplot[draw=black!30!blue] table[x=Step, y=Value]{\modelB};
      \addlegendentry{BCE+GDL};
      %best
      \addplot[draw=black!30!green] table[x=Step, y=Value]{\modelA};
      \addlegendentry{BCE};
    \end{axis}
...
Further out-of-domain results can be found in Figure \ref{fig:mm-pred-long} in the appendix. It shows the scenario where the model tries to predict a much longer sequence than it has been trained for. Such a scenario is simple to realize with this model due to its recurrent encoder-decoder architecture. Therefore, the spatio-temporal decoder component is unrolled for \num{30} time steps in order to verify the long-term stability of predicted frames. It can be seen that the network is able to continuously produce acceptable results, with the exception of the last five frames in Figure \ref{fig:mm-pred-long1}. From about the \nth{30} predicted future frame, the model then starts to produce very noisy outcomes in almost every case.
...
Finally, the stability of the generated future frames is investigated by performing predictions on longer sequences. As it can be seen in Figure \ref{fig:pac-pred-full-long}, some artifacts become visible starting from the \nth{14} forecasted frame. While all moving game characters gradually disappear, the red cherry object is predicted very precisely until the end of the lengthened sequence.
...
\begin{subfigure}{0.5\textwidth}
  ...
  \begin{tikzpicture}[scale=0.5]
    \begin{axis}[
      ...
      \addlegendentry{MAE+GDL \tiny{(96 ConvLSTM feature maps)}};
      \addplot[draw=black!30!blue] table[x=Step, y=Value]{\modelB};
      \addlegendentry{MAE+GDL+SSIM};
      %best
      \addplot[draw=black!30!green] table[x=Step, y=Value]{\modelA};
      \addlegendentry{MAE+GDL+SSIM \tiny{(3-layer ConvLSTM)}};
    \end{axis}
  \end{tikzpicture}
  }
  \caption{}
