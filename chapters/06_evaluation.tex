...
\subsection{Hyperparamter Tuning} \label{sec:hyper_pac}
...
Fortunately, the UCF-101 dataset has been applied in many related studies as well. It is therefore possible to do a more extensive comparison with outcomes of different approaches. Results from an FC-LSTM enocer-decoder network, different convolutional with or without adversarial training and various combinations of loss functions are collected in Figure \ref{fig:ucf-comparison}. When comparing the predicted sequences of non-adversatial networks with the results of our models in the last group at the bottom of the graphic, it can be seen that their outcomes are generally more blurry. However, a clear motion is also not discernible in the forecasted frames of our model. Although the predictions of the adversarial networks do not look close to the ground truth on the long term, they indeed remain very realistic.
