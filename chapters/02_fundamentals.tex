...
\begin{aligned}
\Spvek{\tilde{f}^{(\tau)}; \tilde{i}^{(\tau)}; \tilde{o}^{(\tau)}; \tilde{c}^{(\tau)}} &= W_{h} h^{(\tau-1)} + W_{x} x^{(\tau)} + b \\
\hat{c}^{(\tau)} &= tanh(\tilde{c}^{(\tau)}) \\
C^{(\tau)} &= \sigma(\tilde{f}^{(\tau)}) \odot C^{(\tau-1)} + \sigma(\tilde{i}^{(\tau)}) \odot \hat{c}^{(\tau)} \\
...
\begin{figure}[htpb]
	...
	\caption[Structure of an Autoencoder]{tbd} \label{fig:autoencoder}
\end{figure}
